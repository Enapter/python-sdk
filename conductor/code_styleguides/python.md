...
This document summarizes key rules and best practices used in this project.

## 1. Python Language Rules

- **Linting:** Run `pyflakes` and `mypy` on your code to catch bugs and style issues.
- **Imports:** Use `import x` for packages/modules. Use `from x import y` only when `y` is a subpackage.
...
- **Type Annotations:** Obligatory for all code.

## 2. Python Style Rules

- **Line Length:** Maximum 88 characters.
- **Indentation:** 4 spaces per indentation level. Never use tabs.
- **Blank Lines:** Two blank lines between top-level definitions (classes, functions). One blank line between method definitions. Try to avoid blank lintes within functions and methods.
